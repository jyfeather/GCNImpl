...
class RGCNLayer(nn.Module):
    def __init__(self, in_feat, out_feat, num_rels, num_bases=-1, bias=None, activation=None, is_input_layer=False):
        ...
        self.num_rels = num_rels  # num of relation types
        self.num_bases = num_bases  # num of bases in Equation (3)
        self.bias = bias
        ...
        if self.num_bases < self.num_rels:
            nn.init.xavier_normal_(self.w_comp, gain=nn.init.calculate_gain('relu'))
        if self.bias:
            nn.init.xavier_uniform_(self.bias, gain=nn.init.calculate_gain('relu'))

    def forward(self, g):
        if self.num_bases < self.num_rels:
            # generate weights, W_r, in Equation (3)
        ...
        if self.is_input_layer:
            def message_func(edges):
                # for input layer, matrix multiply can be converted to be a embedding lookup using source node id
...
def load_aifb_data():
    ...
    return val_idx, train_idx, edge_type, edge_norm, labels, data


if __name__ == '__main__':
    val_idx, train_idx, edge_type, edge_norm, labels, data = load_aifb_data()
